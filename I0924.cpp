...
_ ta[maxn], tb[maxn], n, ca, cb, m, dp[maxn], g[maxn];
struct fs {
    _ d, p, c;
...
_ querySuff(_ x);
void addSuff(_ x, _ k);
_ queryPre(_ x);
void addPre(_ x, _ k);
inline void AC() {
        
    memset(ta, -4, sizeof ta);
    memset(tb, -4, sizeof tb);
    ...
    addSuff(m, -m*cb);
    for(_ i = 1; i <= n; ++i) {
        g[i] = dp[i] = max(a[i].c-a[i].p*ca+queryPre(a[i].p), a[i].c+a[i].p*cb+querySuff(a[i].p));
//        addPre(a[i].p, g[i]+a[i].p*ca);
//        addSuff(a[i].p, dp[i]-a[i].p*cb);
        if(a[i].d != a[i+1].d) {
            addSuff(a[i].p, dp[i]-a[i].p*cb);
            _ sv = -1;
            for(_ j = i-1; a[j].d == a[j+1].d; --j) {
                sv = j;
                dp[j] = max(dp[j], a[j].c+a[j].p*cb+querySuff(a[j].p));
//                addPre(a[j].p, dp[j]+a[j].p*ca);
                addSuff(a[j].p, dp[j]-a[j].p*cb);
            }
            if(!~sv) {
                addPre(a[i].p, dp[i]+a[i].p*ca);
                addSuff(a[i].p, dp[i]-a[i].p*cb);
                continue;
            }
            for(_ j = sv; j <= i; ++j) {
                g[j] = max(g[j], a[j].c-a[j].p*ca+queryPre(a[j].p));
                addPre(a[j].p, g[j]+a[j].p*ca);
                dp[j] = max(dp[j], g[j]);
            }
            for(_ j = sv; j <= i; ++j) {
                addSuff(a[j].p, dp[j]-a[j].p*cb);
    ...
    _ s = max(0LL, max(-m*ca+queryPre(m), m*cb+querySuff(m)));
    writeln(s);
...
void addPre(_ x, _ k) {
    for(; x <= big; x += x & -x) ta[x] = max(ta[x], k);
    return;
}

_ queryPre(_ x) {
    _ rv = -1E17;
    for(; x; x -= x & -x) rv = max(rv, ta[x]);
    return rv;
}

void addSuff(_ x, _ k) {
    for(; x; x -= x & -x) tb[x] = max(tb[x], k);
    return;
}

_ querySuff(_ x) {
    _ rv = -1E17;
    for(; x <= big; x += x & -x) rv = max(rv, tb[x]);
    return rv;
}
