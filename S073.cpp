...
constant maxn = 1E2+8;
constant maxm = 1E5+8;
_ n, m, r[maxn][maxm], s = 1E18, c, p[maxn][maxm], t[maxn];

void AC() {
    
    n = read();
    m = read();
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) if(getLw() == 's') p[i][++t[i]] = j;
    for(_ i = 1; i <= n; ++i) {
        _ x = 1;
        for(_ j = 1; j <= m; ++j) {
            if(x <= t[i] && p[i][x] == j) {
            ...
            if(x > t[i]) r[i][j] = m-j+p[i][1];
            else r[i][j] = p[i][x]-j;
            if(x == 1) r[i][j] = min(r[i][j], j+m-p[i][t[i]]);
            else r[i][j] = min(r[i][j], j-p[i][x-1]);
        }
    }
    for(_ i = 1; i <= m; ++i) {
        c = 0;
        for(_ j = 1; j <= n; ++j) c += r[j][i];
        s = min(s, c);
    }
