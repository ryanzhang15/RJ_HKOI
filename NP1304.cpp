...
constant maxn = 1E3+8;
_ n, m, x, t[maxn], in[maxn], s = 1, head[maxn], cnt;
bitset<maxn> p, he[maxn], iq;
struct edge {
    _ to, next;
} g[maxn*maxn];
struct node {
...
void AC() {
    ...
    for(; m--; ) {
        ...
        for(_ j = t[1]; j <= t[x]; ++j) if(!p[j]) for(_ i = 1; i <= x; ++i) if(!he[t[i]][j]) {
            ++in[j];
            addEdge(t[i], j);
        }
    }
    for(_ i = 1; i <= n; ++i) if(!in[i]) q.push((node){i, 1});
    for(; !q.empty(); q.pop()) {
        node t = q.front();
        for(_ i = head[t.p]; i; i = g[i].next) if(!--in[g[i].to]) {
            s = max(s, t.s+1);
            q.push((node){g[i].to, t.s+1});
        }
    }
    writeln(s);
