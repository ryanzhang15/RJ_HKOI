...
inline void AC(str arg) {
    ...
    rd(a, n);
    rd(b, n);
    for(_ i = 1; i <= n; ++i) a[i] = (a[i-1]+a[i])%mod;
    for(_ i = 1; i <= n; ++i) b[i] = (b[i-1]+b[i])%mod;
    for(_ i = 1; i <= n; ++i) s = (s+a[i]*b[i]%mod*(n-i+1))%mod;
    for(_ i = 2; i <= n; ++i) s = (s+a[i]*b[i]%mod*(i-1))%mod;
    for(_ i = 1; i <= n; ++i) {
        ca = (ca+a[i])%mod;
        cb = (cb+b[i])%mod;
    }
    s = (s-ca*cb%mod+mod)%mod;
    for(_ i = 1; i <= n-1; ++i) s = (s+a[i]*b[i]%mod+mod)%mod;
    s = (s+a[n]*b[n]%mod+mod)%mod;
    writeln(s);
