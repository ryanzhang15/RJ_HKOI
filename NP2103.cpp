...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
        ...
        if(a == "Server") {
            if(mp.count(tp)) writeln("FAIL");
            else {
                writeln("OK");
                mp[tp] = i;
            }
        } else {
            if(mp.count(tp)) writeln(mp[tp]);
            else writeln("FAIL");
        }
    }
...
bool ver(str a) {
    ...
    if(sscanf(a.c_str(), "%lld.%lld.%lld.%lld:%lld", &get<0>(tp), &get<1>(tp), &get<2>(tp), &get<3>(tp), &get<4>(tp)) != 5) return true;
    if(get<0>(tp) < 0 || get<0>(tp) > 255 || get<1>(tp) < 0 || get<1>(tp) > 255 || get<2>(tp) < 0 || get<2>(tp) > 255 || get<3>(tp) < 0 || get<3>(tp) > 255 || get<4>(tp) < 0 || get<4>(tp) > 65535) return true;
    str b = to_string(get<0>(tp))+"."+to_string(get<1>(tp))+"."+to_string(get<2>(tp))+"."+to_string(get<3>(tp))+":"+to_string(get<4>(tp));
