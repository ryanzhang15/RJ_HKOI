...
inline void AC() {
    ...
    sort(c+1, c+1+m);
    for(_ i : rg(m)) {
        for(; !q.empty() && q.top().lv < c[i].first; q.pop()) st.insert(q.top().id);
        ++a[*st.begin()];
        q.push((pl){c[i].second, *st.begin()});
        st.erase(st.begin());
    }
    for(_ i : rg(k)) {
        st.insert(i);
    ...
    for(; !q.empty(); ) q.pop();
    for(_ i : rg(k)) {
        for(; !q.empty() && q.top().lv < d[i].first; q.pop()) st.insert(q.top().id);
        ++b[*st.begin()];
        q.push((pl){d[i].second, *st.begin()});
        st.erase(st.begin());
    }
    for(_ i = 1; i <= n; ++i) a[i] += a[i-1];
    for(_ i = 1; i <= n; ++i) b[i] += b[i-1];
    _ s = max(a[n], b[n]);
    for(_ i = 1; i <= n-1; ++i) s = max(s, a[i]+b[n-i]);
    writeln(s);
