...
constant big = 2E3;
constant maxn = big+8;
_ t, k, a[maxn][maxn], b[maxn][maxn], x, y;
...
void AC() {
    ...
    k = read();
    a[1][1] = 1;
    for(_ i = 0; i <= big; ++i) a[i][0] = 1;
    for(_ i = 2; i <= big; ++i) for(_ j = 1; j <= i; ++j) a[i][j] = (a[i-1][j]+a[i-1][j-1])%k;
    for(_ i = 2; i <= big; ++i) {
        for(_ j = 1; j <= i; ++j) b[i][j] = b[i-1][j] + b[i][j-1] - b[i-1][j-1] + !a[i][j];
        b[i][i+1] = b[i][i];
    }
    for(; t--; ) {
        x = read();
