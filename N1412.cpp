...
_ n, m, mxb, l, r, md, s = 1E17, f[maxn], tp, la, ra, ma, sa;
struct edge {
    _ x, y, a, b;
} g[maxm];
...
//    freopen("/Users/ryanzhang/Downloads/P2387_4.in", "r", stdin);
...
inline bool forest() {
    ...
    for(_ i = 1; i <= m; ++i) if(g[i].a <= md && g[i].b <= ma) f[find(g[i].x)] = find(g[i].y);
    if(find(1) == find(n)) {
        s = min(s, ma+md);
        return true;
    }
