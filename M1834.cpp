...
_ n, q, x1, y12, x2, y2, x, l, h;
__int128 a[maxn*4];

void AC() {
    
    scanf("%lld", &n);
    for(_ i = 0; i < n; ++i) a[i] = read();
    for(_ i = n; i < n*4; ++i) a[i] = a[i-n];
    for(_ i = 1; i < n*4; ++i) a[i] += a[i-1];
    for(_ i = 1; i < n*4; ++i) a[i] += a[i-1];
    scanf("%lld", &q);
    for(; q--; ) {
        scanf("%lld%lld%lld%lld", &x1, &y12, &x2, &y2);
        l = y2-y12+1;
        h = x2-x1;
        x = (x1+y2)%n+n+n;
        printf("%lld\n", (_)((__int128)a[x+h]-a[x-1]-a[x-l+h]+a[x-l-1]));
    }
