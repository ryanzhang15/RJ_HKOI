...
_ n, s, t, head[maxn], lst[maxn], pre[maxn], d[maxn], x, cnt, mx, mn, fl[maxn], p, T;
struct edge {
    _ to, cost, dis, next;
} g[maxe];
bitset<maxn> v;
...
bool spfa() {
    ...
    for(; !q.empty(); q.pop()) {
        _ t = q.front();
        v.reset(t);
        for(_ i = head[t]; i; i = g[i].next) if(g[i].cost && d[g[i].to] > d[t] + g[i].dis) {
            d[g[i].to] = d[t] + g[i].dis;
            pre[g[i].to] = t;
            lst[g[i].to] = i;
            fl[g[i].to]= min(fl[t], g[i].cost);
            if(!v[g[i].to]) {
                v.set(g[i].to);
                q.push(g[i].to);
            }
