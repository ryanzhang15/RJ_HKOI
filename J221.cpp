...
inline void AC() {
    ...
    read(a);
    _ p = 0;
    if(!(a[p] >= '0' && a[p] <= '9')) b = a[p++];
    for(; p < a.size() && a[p] >= '0' && a[p] <= '9'; ++p) x = (x<<3)+(x<<1)+(a[p]^48);
    if(p < a.size()) s = a[p];
    
    if(b != 0 && b != 'A' && b != 'B' && b != 'N') invalid();
    if(s != 0 && s != 'A' && s != 'B' && s != 'C' && s != 'P' && s != 'S' && s != 'X') invalid();
    t = (x/10)%10;
    h = (x/100)%10;
    if(t >= 5 && t != 7 && t != 9) invalid();
    if(h > 3) invalid();
    if(s == 'P' && (b == 'N' || h == 3)) invalid();
    
    if(t <= 2) writesc("Downtown");
    ef(t <= 4) writesc("West District");
    ef(t == 7) writesc("North District");
    else writesc("East District");
    
    if(!b && s <= 'C' && !h) writesc("Normal");
    
    if(h == 1) writesc("Cross River");
    ef(h == 2) writesc("Air-conditioned");
    ef(h == 3) writesc("Holiday");
    
    if(s == 'P') writesc("Peak Hour");
    ef(s == 'S') writesc("Special");
    ef(s == 'X') writesc("Express");
    
    if(b == 'A') writesc("Airport");
    ef(b == 'B') writesc("Border");
    ef(b == 'N') writesc("Overnight");
